...
class IntegratedTradingSystem:
    ...
    def predict_all_candles(self, candles_data):
        """
        Generate classification predictions for all candles in the dataset
        This method adds classification columns to the entire DataFrame
        """
        candles_with_predictions = candles_data.copy()

        # Initialize classification columns
        candles_with_predictions['classification_direction'] = np.nan
        candles_with_predictions['classification_prob_down'] = np.nan
        candles_with_predictions['classification_prob_flat'] = np.nan
        candles_with_predictions['classification_prob_up'] = np.nan

        # We need at least training_window + 20 candles to start predictions
        min_candles_needed = max(self.training_window, 100) + 20

        if len(candles_with_predictions) < min_candles_needed:
            print(f"Warning: Need at least {min_candles_needed} candles for predictions. Got {len(candles_with_predictions)}")
            return candles_with_predictions

        # Start predictions from the minimum required index
        ...
        for i in range(start_idx, len(candles_with_predictions)):
            ...
            if len(train_data) > self.training_window:
                model_train_data = train_data.tail(self.training_window).copy()
            else:
                model_train_data = train_data.copy()

            # Create target for training
            target_ret = model_train_data['close'].pct_change().shift(-1)
            model_train_data['target'] = np.select(
                [target_ret < -0.001, target_ret > 0.001],
                [-1, 1], default=0
            )

            # Prepare features for training (shift by 1 to avoid forward-looking bias)
            X_train = model_train_data[self.feature_cols].shift(1).iloc[20:]
            Y_train = model_train_data['target'].iloc[20:]
            ...
            if len(X_train) > 50:
                # Train model
                temp_model = GradientBoostingClassifier(n_estimators=50, max_depth=3, random_state=42)
                temp_model.fit(X_train, Y_train)

                # Make prediction for current candle using features from previous candle
                current_features = candles_with_predictions[self.feature_cols].iloc[i-1:i].fillna(method='ffill').fillna(0)

                if len(current_features) > 0:
                    prediction = temp_model.predict(current_features)[0]
                    probabilities = temp_model.predict_proba(current_features)[0]

                    # Store predictions
                    candles_with_predictions.loc[candles_with_predictions.index[i], 'classification_direction'] = prediction
                    candles_with_predictions.loc[candles_with_predictions.index[i], 'classification_prob_down'] = probabilities[0] if len(probabilities) > 0 else 0
                    candles_with_predictions.loc[candles_with_predictions.index[i], 'classification_prob_flat'] = probabilities[1] if len(probabilities) > 1 else 0
                    candles_with_predictions.loc[candles_with_predictions.index[i], 'classification_prob_up'] = probabilities[2] if len(probabilities) > 2 else 0

            # Print progress every 50 candles
            if i % 50 == 0:
                print(f"Processed {i}/{len(candles_with_predictions)} candles...")
    ...
    def update_and_predict(self, new_tick_data):
        ...
        if self.classification_model is not None and len(self.historical_candles) > 0:
            ...
            self.historical_candles.loc[self.historical_candles.index[-1], 'classification_direction'] = prediction
            self.historical_candles.loc[self.historical_candles.index[-1], 'classification_prob_down'] = probabilities[0] if len(probabilities) > 0 else 0
            self.historical_candles.loc[self.historical_candles.index[-1], 'classification_prob_flat'] = probabilities[1] if len(probabilities) > 1 else 0
            self.historical_candles.loc[self.historical_candles.index[-1], 'classification_prob_up'] = probabilities[2] if len(probabilities) > 2 else 0
    ...
    def get_candles_with_predictions(self):
        """
        Return the historical candles DataFrame with classification predictions
        """
        return self.historical_candles.copy()

    def save_predictions_to_file(self, filepath, file_format='csv'):
        """
        Save candles with predictions to file

        filepath: Path to save the file
        file_format: 'csv', 'parquet', or 'excel'
        """
        df = self.get_candles_with_predictions()
...
if __name__ == "__main__":
    ...
    # Method 1: Real-time processing (incremental updates)
    # results = trading_system.update_and_predict(trade_data)
    # print("Latest prediction:", results)

    # Method 2: Batch processing (generate predictions for all candles)
    # candles = trading_system.process_tick_data_to_candles(trade_data)
    # candles = trading_system.add_all_features(candles)
    # candles_with_predictions = trading_system.predict_all_candles(candles)

    # Save results
    # trading_system.save_predictions_to_file('trading_predictions.csv', 'csv')
    # trading_system.save_predictions_to_file('trading_predictions.parquet', 'parquet')
    ...
    print("- Classification results added as DataFrame columns")
    print("- Batch and real-time processing modes")
    print("- Export to CSV/Parquet/Excel formats")
