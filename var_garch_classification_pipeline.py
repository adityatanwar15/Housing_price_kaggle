...
TRAINING_WINDOW = 100        # Fixed training window (adaptive based on data size)
VAR_MAX_LAGS = 5            # Maximum VAR lags
GARCH_LAGS = 5              # AR lags for GARCH
MAX_PREDICTIONS = None       # Set to number to limit predictions (None = all)
...
TRAINING_WINDOW = min(max(50, total_candles // 10), 500)  # 50-500 candles
VAR_MAX_LAGS = min(max(3, total_candles // 100), 10)      # 3-10 lags
MIN_TRAIN_SIZE = min(max(20, total_candles // 20), 100)   # 20-100 minimum
...
n_preds = min(1000, len(df_diff) // 3)  # Adaptive number of predictions
